...
func HelpUseHerbicideRpc(
	...
	if tile.SeedGrowthInfo.CurrentStage != collections_placed_items.CROP_CURRENT_STATE_IS_WEEDY {
	...
	//update tile status
	tile.SeedGrowthInfo.CurrentStage = collections_placed_items.CROP_CURRENT_STATE_NORMAL
	...
	err = collections_player.IncreaseExperiences(ctx, logger, db, nk, collections_player.IncreaseExperiencesParams{
		UserId: userId,
		Amount: activities.HelpUseHerbicide.ExperiencesGain,
	})
	if err != nil {
		logger.Error(err.Error())
		return "", err
